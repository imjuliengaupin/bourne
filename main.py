...
def is_workflow_ready(agent_context: AgentContext, workflow_plan: Optional[List[Dict[str, Any]]], source_data_connector: Optional[Dict[str, Any]]) -> bool:
    ...
    schema: Dict[str, Union[str, Dict[str, Any], List[Any]]] = source_data_connector.get("expected_schema", {})

    if isinstance(schema, dict) and any(isinstance(value, (dict, list)) for value in schema.values()):
        agent_context.logger.log("Main", "ℹ️ INFO: Detected a complex nested schema, enhanced validation and transformation will be used.")
...
def load_json(logger: Logger, path: Union[str, Path]) -> Optional[Union[Dict[str, Any], List[Dict[str, Any]]]]:
    try:
        with open(path, "r", encoding="utf-8") as file:
            ...
            if logger.debug_mode_enabled:
                logger.log("Main", f"✅ SUCCESS: Loaded configs from {path}")
...
def parse_program_args() -> Namespace:
    ...
    parser.add_argument(
        "--workflow",
        default="json/workflows/default/default.json",
        help="Path to the workflow plan configuration file (defines processing steps and dependencies)."
    )

    parser.add_argument(
        "--connector",
        default="json/connectors/default/single-record.json",
        help="Path to the source data connector configuration file (defines data source, schema, and transformations)."
    )

    parser.add_argument(
        "--debug",
        action="store_true",  # Defaults to False if arg is not provided by user
        help="Enable debug mode with Rich dashboard and real-time logging features."
    )
...
if __name__ == "__main__":
    ...
    main(args.workflow, args.connector, args.debug)
